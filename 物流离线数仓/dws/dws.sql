...
CREATE EXTERNAL TABLE dws_trans_bound_finish_td(
    ...
    COMMENT '物流城转运完成历史至今汇总表'
    PARTITIONED BY (`dt` string COMMENT '统计日期')
    STORED AS ORC
    LOCATION 'hdfs://cdh01:8020/warehouse/fms/dws/dws_trans_bound_finish_td'
    TBLPROPERTIES('orc.compress'='snappy');
...
from (select order_id,
     ...
     (select id,
      ...
      from dim_region_full
      where dt = '2025-07-12') city
     on city_id = city.id
         left join
     (select id,
             name
      from dim_region_full
      where dt = '2025-07-12') province
...
from (select org_id,
     ...
     (select id,
             ...
             parent_id
      from dim_region_full
      where dt = '2025-07-12') province_for_level1
     ...
     (select id,
             ...
             parent_id
      from dim_region_full
      where dt = '2025-07-12') province_for_level2
...
from (select id,
     ...
     (select id,
             ...
             truck_model_type_name
      from dim_truck_full
      where ds = '2025-07-12') truck_info
...
from (select id,
      ...
      where dt = '2025-07-12') trans_finish
         left join
     (select id,
             truck_model_type,
             truck_model_type_name
...
--       where dt >= date_add('2025-07-12', -recent_days + 1)
     ...
     (select id,
             line_name
      from dim_shift_full
      where dt = '2025-07-12') for_line_name
